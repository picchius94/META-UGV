...
chrono.SetChronoDataPath("C:/Users/mv00282/Anaconda3/envs/tfgpu/Library/data/")
...
class MySoilParams (veh.SoilParametersCallback):
    def __init__(self, terrain_types, map_size_x, map_size_y, terrain_params_noise = 0):
            ...
            if sp is None:
                ...
                if terrain_params_noise:
                    for i in range(len(sp)):
                        sp[i] = sp[i] + sp[i]*terrain_params_noise*random.uniform(-1,1) 
                self.list_sp.append(sp)
        self.n_terrains = len(terrain_types)
        self.terrain_types = terrain_types
        self.map_bins = np.linspace(-map_size_x/2,map_size_x/2,self.n_terrains+1)
    def Set(self, x, y):
        idx = int(min(np.digitize(x,self.map_bins),self.n_terrains))
        sp = self.list_sp[idx-1]
...
class simulator:
    def __init__(self, map_name, map_dims, init_loc, init_rot, terrain_type, terrain_params_noise = 0, visualisation = True):
        ...
        if not hasattr(terrain_type, "__iter__") and terrain_type == 23:
            ...
            patch = self.terrain.AddPatch(patch_mat, 
                                      chrono.CSYSNORM, path_map, "terrain",
                                      map_size_x, map_size_y, 0, map_height)
            if self.visualisation:
            ...
            if not hasattr(terrain_type, "__iter__"):
                ...
                if sp is not None:
                    ...
                    if terrain_params_noise:
                        for i in range(len(sp)):
                            sp[i] = sp[i] + sp[i]*terrain_params_noise*random.uniform(-1,1) 
                    self.terrain.SetSoilParameters(sp[0], sp[1], sp[2], sp[3], sp[4], sp[5], sp[6], sp[7])
                else:
                    print("Invalid terrain type")
                    return -1
            else:
                self.my_params = MySoilParams(terrain_type, map_size_x, map_size_y, terrain_params_noise)
        ...
        while (True):
            ...
            if sim_frame % self.data_steps == 0:
                self.data_list.append({"Time": time, "I_Steering": driver.GetSteering(),
                                    ...
                                    "FWD_Speed": speed_rel.x, "Lat_Distance": lat_distance,
